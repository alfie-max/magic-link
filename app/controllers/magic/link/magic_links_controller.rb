  ...
  module Link
    class MagicLinksController < ::ApplicationController
      ...
      def create
        ...
        flash[:notice] = I18n.t('magic_links.check_email')
        render 'authenticate_otp'
      end

      def authenticate
        email = params[:email].presence
        token = params[:sign_in_token].presence
        otp_token = params[:otp_token].presence
        user  = email && (token || otp_token) &&
                Magic::Link.user_class.find_by(email: email)

        # TODO: Handle a different user trying to sign in
        if (token || otp_token) && send("#{Magic::Link.user_class.name.underscore}_signed_in?")
          flash[:alert] = I18n.t('magic_links.already_signed_in')
          redirect_to main_app.send(Magic::Link.after_sign_in_path)
        elsif user && (token_matches?(user) || otp_token_matches?(user)) && token_not_expired?(user)
          flash[:notice] = I18n.t('magic_links.signed_in')
          user.update_columns(sign_in_token: nil, sign_in_token_sent_at: nil, otp_token: nil)
          sign_in user
          redirect_to main_app.send(Magic::Link.after_sign_in_path)
        elsif email && token
      ...
      private
        ...
        def otp_token_matches?(user)
          user.otp_token.eql?(params[:otp_token])
        end
